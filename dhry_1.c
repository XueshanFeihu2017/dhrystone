...
static int init_pmu(unsigned int event_id)
{
...
static void fini_pmu(int fddev)
{
	close(fddev);
}

/* Function: Get PMU counter from hardware */
static long long get_pmu(int fddev)
{
	long long result = 0;
		...
		/* Main and Proc_0 in the Ada version             */
{
	...
	{
		int fddev_val = init_pmu(event_id_array[id]);
		if (fddev_val < 0)
		{
...
#ifdef GET_PMU
		long long cnt_stop = get_pmu(fddev_val);
		fini_pmu(fddev_val);

		printf("PMU Event Name 0x%03x: %llu - %llu = %llu\n", event_id_array[id], cnt_stop, cnt_start, cnt_stop - cnt_start);
	}
#endif
